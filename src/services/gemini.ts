...
export async function getFarmingVoiceAdvice(base64Audio: string, mimeType: string, language: string = 'English'): Promise<string> {
  ...
  If the detected language is one of the supported ones (Hindi, Haryanvi, Bhojpuri, Punjabi, English, Marathi, Bengali, Telugu, Hinglish, etc.), respond in that SAME language. 
...
export async function textToSpeech(text: string, voiceName: 'Kore' | 'Zephyr' | 'Puck' | 'Charon' | 'Fenrir' = 'Kore'): Promise<string | null> {
  ...
  const models = ["gemini-3.1-flash-tts-preview", "gemini-flash-latest"];
...
export async function getSpecializedAdvice({ crop, location, weather, category, language = 'English' }: any): Promise<string> {
  const model = "gemini-3-flash-preview";
  const prompts = {
